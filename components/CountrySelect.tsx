        ...
        <div className="absolute z-[100] mt-1 w-full bg-white dark:bg-stone-800 border border-stone-200 dark:border-stone-700 rounded-xl shadow-2xl overflow-hidden animate-in fade-in slide-in-from-top-1 duration-200">
          <div className="p-2 border-b border-stone-100 dark:border-stone-700 bg-stone-50 dark:bg-stone-900/50">
            <div className="relative">
              ...
              <input type="text" autoFocus value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} placeholder={language === 'fr' ? 'Rechercher...' : 'Search...'} className={`w-full pl-9 pr-3 py-2 bg-white dark:bg-stone-800 border border-stone-200 dark:border-stone-700 rounded-lg focus:ring-[var(--theme-primary)] focus:border-[var(--theme-primary)] outline-none ${fs.lg}`} />
            </div>
          </div>
          <div className="max-h-60 overflow-y-auto no-scrollbar">
            {filteredCountries.length > 0 ? (
              filteredCountries.map((country) => (
                <button key={country} type="button" onClick={() => handleSelect(country)} className={`w-full text-left px-4 py-3 hover:bg-[var(--theme-bg-soft)] dark:hover:bg-rose-900/20 transition-colors flex items-center justify-between ${fs.lg} ${value === country ? 'text-[var(--theme-primary)] dark:text-rose-400 font-bold bg-[var(--theme-bg-soft)]/50 dark:bg-rose-900/10' : 'text-stone-700 dark:text-stone-300'}`}>{country}{value === country && <div className="w-1.5 h-1.5 rounded-full bg-[var(--theme-primary)] dark:bg-rose-400"></div>}</button>
              ))
            ) : (
              <div className="px-4 py-8 text-center">
                <p className={`text-stone-400 italic mb-3 ${fs.base}`}>{language === 'fr' ? 'Aucun pays répertorié' : 'No country listed'}</p>
                {onEdit && ( <button type="button" onClick={() => { onEdit(); setIsOpen(false); }} className={`px-4 py-2 bg-[var(--theme-bg-soft)] dark:bg-rose-900/20 text-[var(--theme-primary)] dark:text-rose-400 rounded-lg font-bold uppercase tracking-widest border border-[var(--theme-border)] dark:border-rose-900/30 ${fs.label}`}>{t('add_country')}</button> )}
              </div>
            )}
          </div>
        </div>
