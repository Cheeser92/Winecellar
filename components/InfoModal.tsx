...
export const InfoModal: React.FC<InfoModalProps> = ({ isOpen, onClose, fontSize = 'medium' }) => {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-6">
      ...
      <div className="relative bg-white dark:bg-stone-900 rounded-3xl w-full max-w-xs p-8 shadow-2xl flex flex-col items-center text-center transition-all animate-in zoom-in-95 duration-200">
        ...
        <h2 className={`font-serif font-bold text-stone-900 dark:text-stone-100 mb-1 ${fs.xl}`}>My Wine Cellar</h2>
        <p className={`font-bold text-stone-400 dark:text-stone-500 uppercase tracking-widest mb-4 ${fs.base}`}>v1.0 • 2025</p>
        <div className="space-y-4 mb-6">
            <p className={`text-stone-600 dark:text-stone-400 leading-relaxed ${fs.lg}`}>Développé entièrement avec <br/><span className="font-bold text-rose-900 dark:text-rose-400">Google AI Studio</span></p>
        ...
        <div className="w-full mb-8 p-5 bg-white dark:bg-stone-800/50 rounded-2xl border border-stone-100 dark:border-stone-800 shadow-sm">
          <p className={`text-stone-500 dark:text-stone-400 leading-relaxed mb-4 text-center ${fs.base}`}>Un commentaire, une remarque, un remerciement ? Envoyez un message. Les demandes de modifications seront traitées en fonction de mon temps libre.</p>
          <button onClick={() => window.open('mailto:cheeser92@gmail.com', '_blank')} className={`w-full bg-stone-50 dark:bg-stone-800 text-rose-900 dark:text-rose-400 font-bold py-3 px-4 rounded-xl border border-rose-100 dark:border-rose-900/30 hover:bg-rose-100 dark:hover:bg-rose-900/20 transition flex items-center justify-center gap-2 shadow-sm active:scale-95 ${fs.lg}`}><Mail size={16} />Me contacter</button>
        </div>
        <button onClick={onClose} className={`w-full bg-white dark:bg-stone-800 text-stone-800 dark:text-stone-200 font-bold py-3 rounded-xl hover:bg-stone-100 dark:hover:bg-stone-700 transition shadow-sm ${fs.lg}`}>Fermer</button>
      </div>
    </div>
  );
};
