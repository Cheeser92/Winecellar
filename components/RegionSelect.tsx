                ...
                {filteredRegions.length > 0 ? (
                  <div className="pb-1">
                    ...
                    {filteredRegions.map(region => (
                      <button key={`${country}-${region}`} type="button" onClick={() => handleSelect(region)} className={`w-full text-left px-4 py-3 hover:bg-rose-50 dark:hover:bg-rose-900/20 transition-colors flex items-center justify-between ${fs.lg} ${value === region ? 'text-rose-900 dark:text-rose-400 font-bold bg-rose-50/50 dark:bg-rose-900/10' : 'text-stone-700 dark:text-stone-300'}`}>{region}{value === region && <div className="w-1.5 h-1.5 rounded-full bg-rose-900 dark:bg-rose-400"></div>}</button>
                    ))}
                  </div>
                ) : (
                  <div className="px-4 py-8 text-center">
                    <p className={`text-stone-400 italic mb-3 ${fs.base}`}>{language === 'fr' ? 'Aucune région répertoriée' : 'No region listed'}</p>
                    {onEdit && ( <button type="button" onClick={() => { onEdit(); setIsOpen(false); }} className={`px-4 py-2 bg-rose-50 dark:bg-rose-900/20 text-rose-900 dark:text-rose-400 rounded-lg font-bold uppercase tracking-widest border border-rose-200 dark:border-rose-900/30 ${fs.label}`}>{language === 'fr' ? 'Ajouter une région' : 'Add a region'}</button> )}
                  </div>
                )}
              </div>
            </>
